...
CREATE UNIQUE INDEX UQ_CARDS_ONE_ACTIVE_PER_CUSTOMER ON CARDS (CASE WHEN IS_ACTIVE = 1 THEN CUSTOMER_ID END);
...
-- Note:
-- CUSTOMERS(USER_ID) and ACCOUNTS(ACCOUNT_NUMBER) already have UNIQUE constraints,
-- so Oracle creates backing indexes automatically. Creating manual indexes on those
-- same column lists causes ORA-01408.
