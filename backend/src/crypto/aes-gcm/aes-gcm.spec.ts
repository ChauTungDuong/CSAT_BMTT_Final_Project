...
describe('Pure TypeScript AES-256-GCM', () => {
  it('should encrypt and decrypt mathematically matching Node standard crypto', () => {
    ...
    const plaintext = Buffer.from(
      'Hello World Secret Data Authentication!!',
      'utf8',
    );

    // 1. Encrypt with Node standard library
    const cipher = crypto.createCipheriv('aes-256-gcm', key, iv);
    const nodeCiphertext = Buffer.concat([
    ...
    const { ciphertext: tsCiphertext, authTag: tsAuthTag } = encryptGCM(
      key,
      iv,
      plaintext,
    );
    ...
    const tsDecrypted = decryptGCM(
      key,
      iv,
      Math.random() > 0.5 ? nodeCiphertext : tsCiphertext,
      tsAuthTag,
    );
    expect(Buffer.from(tsDecrypted).toString('utf8')).toEqual(
      'Hello World Secret Data Authentication!!',
    );
  });
